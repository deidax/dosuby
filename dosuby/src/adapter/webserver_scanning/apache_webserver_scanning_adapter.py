...
class ApacheWebServerScanningAdapter(WebserverEnumerationAdapter):
    """Adapter for scanning Apache web servers and detecting versions"""
    
    def __init__(self) -> None:
        super().__init__()
        self.server_name = "Apache"
    
    def _process(self) -> dict:
        """
        Process Apache server detection and version extraction
        ...
        try:
            ...
            # Apache specific pattern
            apache_pattern = r'Apache(?:/|\ )([\d\.]+)'
            match = re.search(apache_pattern, server_header, re.IGNORECASE)
            ...
            if match:
                ...
                # Apache is present but version not detected
                return {
                    "detected": True,
                    "server": "Apache",
                    "version": None,
            ...
            print(f"Error in Apache detection: {str(e)}")
            return {"detected": False}
