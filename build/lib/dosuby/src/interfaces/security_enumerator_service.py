...
class SecurityEnumeratorService(ABC):
    ...
    def build_enumerator(self, target: Target):
        """This method will build the security enumerator to use. example:\n
        socket_port_scanning = SockerPortScanningAdapter()\n
        port_scanning_usecase = PortsEnumerationUseCase(security_enumerator=socket_port_scanning)\n
        result = port_scanning_usecase.execute(target=target)\n
        return result\n
