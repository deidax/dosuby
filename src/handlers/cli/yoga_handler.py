...
from src.services.yoga.cli.yoga_cli_service import YogaCliService
...
class YogaHandler(HandlerCli):
    
    
    def run_service(self, uri, success_response: SuccessResponse):
        try:
            return YogaCliService(success_response=success_response).read(uri=uri)
